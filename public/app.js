...
function renderAnalysis(stock, rows, institutionalRows = []) {
  ...
  ${entry ? `<section class="entry-strategy" aria-label="模型最佳進場計畫"><div class="entry-strategy-heading"><div><p class="eyebrow">MODEL ENTRY PLAN</p><h4>模型最佳進場點</h4></div><span class="entry-status">${esc(entry.status)}</span></div><div class="entry-strategy-grid"><div><span>分批進場區間</span><strong>${formatNumber(entry.zoneLow)}–${formatNumber(entry.zoneHigh)} 元</strong></div><div><span>突破追蹤價</span><strong>${formatNumber(entry.breakoutPrice)} 元</strong></div><div><span>模型失效價</span><strong>${formatNumber(entry.invalidationPrice)} 元</strong></div></div><p>${esc(entry.rationale)}</p><small>${esc(entry.method)}；模型區間不等同保證成交或報酬。</small></section>` : ""}
