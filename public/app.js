...
$("recommendationMeta").innerHTML = `資料日 <strong>${esc(recommendations.dataAsOf)}</strong> · 產業觀察 ${esc(recommendations.focusWindow.from)} 至 ${esc(recommendations.focusWindow.to)}<br><span>買進門檻：總分至少 ${formatNumber(recommendations.decisionRules.buyMinScore)} 分且無重大風險旗標。${esc(recommendations.disclaimer)}</span>`;
